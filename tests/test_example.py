...
@pytest.mark.unit
def test_planner_decomposes_goal_into_tasks() -> None:
    ...
    _goal = "Promote new sneaker drop to Gen-Z audience"
...
@pytest.mark.unit
def test_planner_creates_task_with_state_version() -> None:
...
@pytest.mark.unit
async def test_worker_executes_content_generation_task(
    sample_task: dict, mock_mcp_client: object
) -> None:
...
@pytest.mark.unit
async def test_worker_retrieves_memories_before_generation(
    ...
    # mock_mcp_client.call_tool.return_value = [{"content": "Past post"}]
...
@pytest.mark.unit
def test_judge_auto_approves_high_confidence() -> None:
    ...
    _result = {"task_id": "test", "caption": "Great content", "confidence_score": 0.95}
...
@pytest.mark.unit
def test_judge_escalates_medium_confidence() -> None:
    """Test that Judge escalates results with confidence 0.70-0.90 to HITL.

    Spec: functional.md - Story 2.2: Confidence-based routing
    """
    # Given: A result with medium confidence
    _result = {"task_id": "test", "caption": "Decent content", "confidence_score": 0.82}
...
@pytest.mark.unit
def test_judge_rejects_low_confidence() -> None:
    """Test that Judge rejects results with confidence < 0.70.

    Spec: functional.md - Story 2.2: Confidence-based routing
    """
    # Given: A result with low confidence
    _result = {"task_id": "test", "caption": "Poor content", "confidence_score": 0.65}
...
@pytest.mark.unit
def test_judge_enforces_occ_state_version() -> None:
    ...
    _task = {"state_version": 42, "agent_id": "test"}
    _result = {"task_id": "test", "confidence_score": 0.95}
...
@pytest.mark.unit
async def test_trend_discovery_filters_by_relevance() -> None:
...
async def test_full_content_generation_workflow() -> None:
...
@pytest.mark.integration
async def test_mcp_twitter_server_posts_tweet() -> None:
